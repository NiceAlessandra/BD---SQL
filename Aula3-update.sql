...
select * from imagens;
update imagens set arquivo='indisponivel'
where arquivo<>'frente';
# between -> entre somente para números

update produtos set descricao='tá barato pra caramba'
where preco between 5 and 15;

update produtos set descricao='tá barato demais'
where preco>=5 and preco<=15;
...
select * from clientes;
update clientes set email='alerando'
where cod_cidade=1 or cod_cidade=2 or cod_cidade=3;

update clientes set email='pedrinho@gmail.com'
where cod_cidade=1 or cod_cidade=2 or cod_cidade=3;
-- like - permite usar caracteres especiais
update produtos set preco=preco+5
where nome like 'p%'; #procura pelo caracter p no inicio da palavra
select * from produtos;
update produtos set preco=preco+5
where nome like '%n'; #procura pelo caracter n no fim da palavra

update produtos set preco=preco+5
where nome like '%a%'; #procura pelo caracter a em qualquer posição da palavra

select * from subcategorias;
#concat('jo','ao') 
#concatenação 

update subcategorias set nome=concat(nome,'1')
where nome like '__a%';
update carrinhosprodutos set quantidade=quantidade+5 
where preco <>30.50;

-- vamos utilizar o comando delete 
-- curso pratico de sql com mysql aula 09 delete
